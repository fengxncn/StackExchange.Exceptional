...
public class ErrorListPage(
    ErrorStore store, 
    ExceptionalSettingsBase settings, 
    string baseURL, 
    List<Error> errors) : WebPage(null, settings, store, baseURL, "Error Log")
{
    private List<Error> Errors { get; } = errors.OrderByDescending(e => e.LastLogDate ?? e.CreationDate).ToList();
