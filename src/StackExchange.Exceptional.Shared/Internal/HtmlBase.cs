...
/// </summary>
public abstract partial class HtmlBase
{
    /// <summary>
    /// Known ServerVariable keys to hide when rendering
    /// </summary>
    protected static readonly HashSet<string> HiddenHttpKeys =
    [
        "ALL_HTTP",
        "ALL_RAW",
        "HTTP_CONTENT_LENGTH",
    ...
    /// </summary>
    protected static readonly HashSet<string> DefaultHttpKeys =
    [
        "APPL_MD_PATH",
        "APPL_PHYSICAL_PATH",
        "GATEWAY_INTERFACE",
    ...
    private static readonly Regex _sanitizeUrl = GetSanitizeUrlRegex();
    /// <summary>
    ...
    protected static string Linkify(string s, string color = "#3D85B0")
    {
        if (s.IsNullOrEmpty())
        {
            return string.Empty;
        ...
        if (GetLinkRegex().IsMatch(s))
        {
            s = s.UrlDecode();
        }

        if (GetUrlRegex().IsMatch(s))
        {
    ...
    [GeneratedRegex(@"[^-a-z0-9+&@#/%?=~_|!:,.;\*\(\)\{\}]", RegexOptions.IgnoreCase | RegexOptions.Compiled, "zh-CN")]
    private static partial Regex GetSanitizeUrlRegex();
    [GeneratedRegex("%[A-Z0-9][A-Z0-9]")]
    private static partial Regex GetLinkRegex();
    [GeneratedRegex("^(https?|ftp|file)://")]
