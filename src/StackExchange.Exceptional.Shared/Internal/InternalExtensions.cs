...
public static class InternalExtensions
{
    ...
    public static string ToShortTypeName(this string type)
    {
        ...
        var shortType = lastIndex > -1 && lastIndex + 1 < type.Length ? type[(lastIndex + 1)..] : type;

        const string suffix = "Exception";

        if (shortType.EndsWith(suffix) && shortType != suffix)
            return shortType[..^suffix.Length];
        return shortType;
    }
    ...
    public static string ResolvePath(this string path)
    {
        ...
        if (path.StartsWith(@"~\"))
            return AppDomain.CurrentDomain.GetData("APPBASE") + path[2..];
        return path;
    }
