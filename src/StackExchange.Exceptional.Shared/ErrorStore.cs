...
public abstract class ErrorStore(ErrorStoreSettings settings)
{
    ...
    /// </summary>
    public ErrorStoreSettings Settings { get; } = settings;
    ...
    private static void ProcessNotifications(Error error, Guid originalGuid)
    {
    ...
    internal static ErrorStore Get(ErrorStoreSettings settings)
    {
        ...
        Type match = (KnownStoreTypes.Find(s => s.Name == settings.Type + nameof(ErrorStore))
                  ?? KnownStoreTypes.Find(s => s.Name.Contains(settings.Type))) ?? throw new Exception("Could not find error store type: " + settings.Type);
        try
        {
    ...
    private static List<Type> GetErrorStores()
    {
        ...
        try
        {
            ...
            var assemblyUri = new Uri(Assembly.GetExecutingAssembly().Location);
