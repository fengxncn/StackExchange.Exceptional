...
/// </summary>
public partial class ExceptionalMiddleware
{
    ...
    public static async Task HandleRequestAsync(HttpContext context)
    {
        ...
        var match = MatchRegex().Match(context.Request.Path);
        ...
        IEnumerable<Guid> GetFormGuids()
        {
            try
            {
                return context.Request.Form["ids"].Select(Guid.Parse);
            }
            catch
            {
                return [];
