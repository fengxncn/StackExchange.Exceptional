...
public abstract class StoreBaseTest(ITestOutputHelper output) : BaseTest(output)
{
    ...
    public async Task DeleteErrorsAsync()
    {
        ...
        Assert.True(await store.DeleteAsync([error.GUID, error2.GUID]).ConfigureAwait(false));
    ...
    public async Task ProtectErrorsAsync()
    {
        ...
        Assert.True(await store.ProtectAsync([error.GUID, error2.GUID]).ConfigureAwait(false));
    ...
    protected static Error GetBasicError(string message, ErrorStore store) 
        => new(new Exception(message), GetSettings(store));

    protected static TestSettings GetSettings(ErrorStore store)
    {
